...
class TrajectoryViewModel: ObservableObject {
    ...
    var totalTimeString: String {
        guard let lastPoint = trajectoryPoints.last,
              let firstPoint = trajectoryPoints.first else {
            return "00:00:00"
        }
           ...
           let positions = try? JSONDecoder().decode([AntennaPositionData].self, from: data) {
            ...
            antennaPositions = positions.enumerated().map { index, position in
                AntennaVisualization(
                    ...
                    screenPosition: convertToScreenPosition(position.realWorldPosition),
    ...
    private func applyFilters() {
        trajectoryPoints = allTrajectoryPoints.filter { point in
            ...
            point.timestamp >= startTimeFilter &&
            point.timestamp <= endTimeFilter
        }
    }
    ...
    private func updateCurrentPosition() {
        guard hasTrajectoryData,
              currentTimeIndex < Double(trajectoryPoints.count) else {
            currentPosition = nil
            return
        }
    ...
    func exportTrajectoryData() {
        ...
        panel.nameFieldStringValue = "\(selectedSession.fileName)_trajectory.csv"
        ...
        let fileName = "\(selectedSession.fileName)_trajectory.csv"
    ...
    private func generateCSVContent() -> String {
        ...
        for point in trajectoryPoints {
            ...
            csv += "\(timestamp),\(point.position.x),\(point.position.y),\(point.position.z),\(point.accuracy),\(point.speed)\n"
        }
