...
struct CalibrationUsecaseTests {
    ...
    func addCalibrationPoint_正常ケース() throws {
        ...
        #expect(usecase.calibrationStatus == .collecting)
    }
    ...
    func performCalibration_成功ケース() async throws {
        ...
        // Assert
        #expect(usecase.calibrationStatus == .completed, "キャリブレーション状態がcompletedになるべきです")
        #expect(usecase.lastCalibrationResult != nil, "キャリブレーション結果が存在すべきです")
        ...
        #expect(usecase.isCalibrationValid(for: antennaId), "キャリブレーションが有効になるべきです")
    }
    ...
    func performCalibration_不十分なポイント数でエラー() async throws {
        ...
        // Assert
        #expect(usecase.calibrationStatus == .failed)
        #expect(usecase.lastCalibrationResult != nil)
        #expect(usecase.lastCalibrationResult?.success == false)
        #expect(usecase.errorMessage != nil)
    }
    ...
    func performCalibration_空のアンテナIDでエラー() async throws {
        ...
        #expect(usecase.lastCalibrationResult != nil)
        #expect(usecase.lastCalibrationResult?.success == false)
        #expect(usecase.errorMessage?.contains("アンテナID") == true)
    }
    ...
    func performCalibration_無効な座標値でエラー() async throws {
        ...
        // Assert
        #expect(usecase.calibrationStatus == .failed, "無効な座標値でキャリブレーションが失敗すべきです")
        #expect(usecase.lastCalibrationResult != nil, "キャリブレーション結果が存在すべきです")
        ...
        let hasRelevantError = result.errorMessage?.contains("無効な座標値") == true ||
            ...
            usecase.calibrationStatus == .failed
    ...
    func performCalibration_重複する基準座標でエラー() async throws {
        ...
        // Assert
        #expect(usecase.calibrationStatus == .failed, "重複する基準座標でキャリブレーションが失敗すべきです")
        #expect(usecase.lastCalibrationResult != nil, "キャリブレーション結果が存在すべきです")
    ...
    func applyCalibratedTransform_キャリブレーション済み() async throws {
        ...
        #expect(usecase.isCalibrationValid(for: antennaId), "キャリブレーションが有効でない場合、変換をテストできません")
        ...
        if usecase.isCalibrationValid(for: antennaId) {
    ...
    func clearCalibrationData_特定アンテナ() throws {
        ...
        #expect(!usecase.isCalibrationValid(for: antennaId))
    }
    ...
    func clearCalibrationData_全データ() throws {
        ...
        // Assert
        #expect(usecase.currentCalibrationData.isEmpty)
        #expect(usecase.calibrationStatus == .notStarted)
        #expect(usecase.lastCalibrationResult == nil)
    }
    ...
    func getCalibrationStatistics() async throws {
        ...
        for antenna in antennas {
            ...
            // キャリブレーションが成功したかチェック
            if usecase.isCalibrationValid(for: antenna) {
